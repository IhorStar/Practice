...
public class UnblockUserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int userId = Integer.parseInt(request.getParameter("blockUser"));

		UserDAO userDAO = DAOFactory.getInstance().getUserDAO();
		try {
			User user = userDAO.getUserById(userId);
			user.setStatus(ACTIVE_USER);
			...
			session.setAttribute("success", "User.successfully.unblocked");
			...
			request.setAttribute("error", "Failed.to.unblock.user");
